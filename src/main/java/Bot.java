...
public class Bot extends ListenerAdapter
{
    ...
    public void onMessageReceived(MessageReceivedEvent e)
    {
        ...
        String serv = e.getGuild().getName();
        ...
        if(Chan.toLowerCase().contains("bot")&&!disabled)
        {
            if(messageText.startsWith("-rate:user"))
            {
                ...
                if(true)
                {
                    ...
                    for (TextChannel chn:channels)
                    {
                        ...
                        if((!chn.getName().startsWith("_")&&!chn.getName().toLowerCase().contains("bot")&&!chn.getParent().getName().startsWith("-")&&!chn.getParent().getName().toLowerCase().contains("bot"))&&(chn.getGuild().equals(e.getGuild()) || messageText.contains("-ignore:server") || messageText.contains("-ignore:serv")))
                        {
                            try {
            ...
            if(messageText.startsWith("-rater:help"))
            {
                ...
                {
                    Message msgRate = e.getChannel().sendMessage(Messages.helpImage).complete();
                } else if(messageText.startsWith("-rater:help -rate:user"))
                {
                    Message msgRate = e.getChannel().sendMessage(Messages.helpUser).complete();
                } else {
            ...
            )
            {
                ...
                if(emoteRate)
                {
                    ...
                    rateImage(ico,e,outputFile,outputLegacy,decimal,false);
                } else {
                            ...
                            try {
                                if(attach.getFileName().endsWith(".gif"))
                                {
                                    ...
                                    for (int img = 0; img<=reader.getNumImages(false); img++)
                                    {
                                        ...
                                        GifRating+=rateImage(thisImg.getScaledInstance(reader.getWidth(img),reader.getHeight(img),0),e,false,false,true,true);
                                        GifRating/=1.5;
                                    }
                                    ...
                                    /*try {
                                        ...
                                        for (int image=0; image < images; image++)
                                        {
                                            ...
                                            GifRating+=rateImage(thisImg.getScaledInstance(gifImageReader.getWidth(image),gifImageReader.getHeight(image),0),e,false,false,true,true);
                                            GifRating/=1.5;
                                        }
                                    } catch (IOException err) {}*/
                                    ...
                                    rateImage(img,e,outputFile,outputLegacy,decimal,false);
                                }
                            } catch (IOException err)
                            {
                    ...
                    if(!hasImage)
                    {
                        Message msgRate = e.getChannel().sendMessage("Please attach an image.").complete();
                    }
                }
            }
        }
